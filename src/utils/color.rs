...
impl Color {
    pub fn from_bytes(bytes: &[u8]) -> &[Self] {
        if bytes.len() % std::mem::size_of::<Color>() != 0 {
            panic!("Color slices can only be initializing with a multiple of 4 byte slices");
        }
    ...
    pub fn from_hex_string(hex: &str) -> Result<Color> {
        if hex.len() != 6 && hex.len() != 8 {
            return Err(anyhow!(
                "Hex string has not the right length. Expected 6 or 8 got {}",
                hex.len()
            ));
        }
    ...
    pub fn as_hsl(&self) -> HslColor {
        ...
        // The color conversion moves array every value into the [0,1] number space.
...
impl From<HslColor> for Color {
    fn from(v: HslColor) -> Self {
        ...
        // The input for this algorithm expects all the h, s and l value in the 
        // range [0,1].
        let h = v.h / 360f64;
        let s = v.s / 100f64;
        let l = v.l /100f64;
